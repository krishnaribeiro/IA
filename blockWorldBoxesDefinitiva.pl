...
init_state :-
   retractall(on(_, _)),
    ...
    assertz(on(blockA, (0, 1))),
    assertz(on(blockB, (1, 1))),
    assertz(on(blockD, (3, 0))),
    assertz(on(blockD, (4, 0))),
    assertz(on(blockD, (5, 0))),
    assertz(clear(0, 2)),
    assertz(clear(1, 2)),
    assertz(clear(2, 0)),
    assertz(clear(2, 1)),
...
% Função principal para inicializar o estado 3
init_state_Sit3 :-
   retractall(on(_, _)),
    retractall(clear(_, _)),
    assertz(on(blockC, (0, 0))),
...
execute :-
    ...
    verify_state.
% Inicializar estado e executar a movimentação S0 para S1
executeFirst :-
    init_state,
    moveFloor(blockB, (1, 1) , (2, 0)),
    verify_state.
% Inicializar estado e executar a movimentação S0 para S2
executeSecond :-
    init_state,
    moveFloor(blockB, (1, 1) , (2, 0)),
    verify_state,
    move(blockA, (0,1),blockB,(2,1)),
    verify_state.
executeThird :-
    init_state,
    moveFloor(blockB, (1, 1) , (2, 0)),
...
executeFith :-
    ...
    move(blockA, (0,1),blockB,(2,1)),
    verify_state,
    move(blockC, (0,0),blockD,(3,1)),
    verify_state,
    move(blockA, (2,1),blockC,(4,2)),
    verify_state,
    move(blockB, (2,0),blockC,(5,2)),
    verify_state.
executeFirstSit3 :-
    init_state_Sit3,
    verify_state,
    move(blockD, (3,1),blockC,(1,1)),
    verify_state.
executeSecondSit3 :-
    init_state_Sit3,
    verify_state,
